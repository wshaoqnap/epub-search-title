...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function addFloatingButtons() {
    ...
    Object.assign(up.style, { position: 'fixed', top: `${leftRect.top + margin}px`, left: `${leftPos}px` });
    container.appendChild(up);

    const down = createBtn('▼', '回到底部', () => { left.scrollTop = left.scrollHeight; });
    Object.assign(down.style, { position: 'fixed', top: `${leftRect.top + leftRect.height - btnSize - margin}px`, left: `${leftPos}px` });
  ...
  left.addEventListener('click', e => {
    ...
    if (!hasClickedCell) { toggleRightPanel(); divider.style.display = 'block'; hasClickedCell = true; positionFloatingButtons(); }
    else if (word === lastWordSelected) { toggleRightPanel(); divider.style.display = right.style.display === 'none' ? 'none' : 'block'; positionFloatingButtons(); }
    else if (right.style.display === 'none') { toggleRightPanel(); divider.style.display = 'block'; positionFloatingButtons(); }
